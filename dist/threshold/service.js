...
import { getMicroThreshold, getProratedThreshold, getVatThreshold, WARNING_THRESHOLD_PERCENT, } from "./models";
// ============================================================================
// Service Tag
// ============================================================================
export class ThresholdMonitoringService extends Effect.Tag("@accounting/ThresholdMonitoringService")() {
}
// ============================================================================
// Service Implementation
// ============================================================================
export const ThresholdMonitoringServiceLayer = Layer.effect(ThresholdMonitoringService, Effect.gen(function* () {
    const dataPort = yield* AccountingDataPort;
    const zeroMonetary = () => monetary({ amount: 0, currency: EUR });
    const getYearlyRevenue = (userId, year) => Effect.gen(function* () {
        const period = {
    ...
    const getThresholdStatus = (userId, activityType, year, activityStartDate) => Effect.gen(function* () {
        const ytdRevenue = yield* getYearlyRevenue(userId, year);
        const previousYearRevenue = yield* getYearlyRevenue(userId, year - 1);
        const microThreshold = getProratedThreshold(getMicroThreshold(activityType), activityStartDate, year);
        const vatThreshold = getProratedThreshold(getVatThreshold(activityType), activityStartDate, year);
        const previousMicroThreshold = getProratedThreshold(getMicroThreshold(activityType), activityStartDate, year - 1);
        const microPercentage = microThreshold.amount > 0
            ? Math.round((ytdRevenue.amount / microThreshold.amount) * 100)
            : 0;
        const vatPercentage = vatThreshold.amount > 0 ? Math.round((ytdRevenue.amount / vatThreshold.amount) * 100) : 0;
