...
export function computeSIG(tb) {
    ...
    const productionVendue = add(add(add(add(add(add(rev("701"), rev("702")), rev("703")), rev("704")), rev("705")), rev("706")), rev("708"));
    ...
    const resultatExploitation = subtract(add(add(add(excedentBrutExploitation, autresProduits), reprisesExploitation), transfertsChargesExpl), add(autresCharges, dotationsExploitation));
    ...
    const resultatNet = subtract(add(resultatCourantAvantImpots, resultatExceptionnel), add(participation, impotBenefices));
    return {
        margeCommerciale,
        productionExercice,
        valeurAjoutee,
