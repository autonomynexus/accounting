...
import { getAccountPrefixBalance, getDebitBalance, getCreditBalance, } from "../engine/computations.js";
const ZERO = monetary({ amount: 0, currency: EUR });
// ============================================================================
// Helpers
// ============================================================================
function actifLine(label, brut, amort) {
    return {
        label,
        brut,
...
function passifLine(label, montant) {
    return { label, montant };
}
// ============================================================================
// 2050 — Bilan Actif
// ============================================================================
export function computeForm2050(tb) {
    ...
    const immosIncorp = actifLine("Immobilisations incorporelles", immosIncorpBrut, add(immosIncorpAmort, immosIncorpDeprec));
    ...
    const immosCorp = actifLine("Immobilisations corporelles", immosCorpBrut, add(immosCorpAmort, immosCorpDeprec));
    ...
    // Stocks (31-38)
    const stocksBrut = add(add(add(getDebitBalance(tb, "31"), getDebitBalance(tb, "32")), add(getDebitBalance(tb, "33"), getDebitBalance(tb, "34"))), add(add(getDebitBalance(tb, "35"), getDebitBalance(tb, "36")), add(getDebitBalance(tb, "37"), getDebitBalance(tb, "38"))));
    ...
    const clientsBrut = add(add(getDebitBalance(tb, "411"), getDebitBalance(tb, "413")), add(getDebitBalance(tb, "416"), getDebitBalance(tb, "418")));
    ...
    // Autres créances
    const autresCreancesBrut = add(add(getDebitBalance(tb, "409"), getDebitBalance(tb, "425")), add(add(getDebitBalance(tb, "441"), getDebitBalance(tb, "443")), add(getDebitBalance(tb, "4456"), getDebitBalance(tb, "46"))));
    ...
    const dispoBrut = add(add(getDebitBalance(tb, "50"), getDebitBalance(tb, "51")), add(getDebitBalance(tb, "53"), getDebitBalance(tb, "54")));
    ...
    const totalActif = actifLine("TOTAL ACTIF", add(totalActifImmoBrut, totalActifCircBrut), add(totalActifImmoAmort, totalActifCircAmort));
    return {
        _tag: "Form2050",
...
// 2051 — Bilan Passif
// ============================================================================
export function computeForm2051(tb) {
    ...
    const totalCapitauxPropres = passifLine("Total capitaux propres", add(add(add(capitalSocial.montant, reserves.montant), reportANouveau.montant), add(resultatExercice.montant, subvInvest)));
    // Provisions
    const provisions = passifLine("Provisions pour risques et charges", add(getCreditBalance(tb, "14"), getCreditBalance(tb, "15")));
    // Emprunts et dettes financières
    const emprunts = passifLine("Emprunts et dettes financières", add(add(getCreditBalance(tb, "16"), getCreditBalance(tb, "17")), getCreditBalance(tb, "519")));
    // Dettes fournisseurs
    const dettesFournisseurs = passifLine("Dettes fournisseurs et comptes rattachés", add(getCreditBalance(tb, "401"), add(getCreditBalance(tb, "403"), add(getCreditBalance(tb, "404"), getCreditBalance(tb, "408")))));
    // Dettes fiscales et sociales
    // Note: prefix "44" already includes account 447, no need to add it separately
    const dettesFiscalesSociales = passifLine("Dettes fiscales et sociales", add(add(getCreditBalance(tb, "42"), getCreditBalance(tb, "43")), getCreditBalance(tb, "44")));
    // Autres dettes
    const autresDettes = passifLine("Autres dettes", add(add(getCreditBalance(tb, "419"), getCreditBalance(tb, "455")), getCreditBalance(tb, "46")));
    // Produits constatés d'avance
    const produitsConstates = passifLine("Produits constatés d'avance", getCreditBalance(tb, "487"));
    const totalDettes = passifLine("Total dettes", add(add(add(emprunts.montant, dettesFournisseurs.montant), dettesFiscalesSociales.montant), add(autresDettes.montant, produitsConstates.montant)));
    const totalPassif = passifLine("TOTAL PASSIF", add(add(totalCapitauxPropres.montant, provisions.montant), totalDettes.montant));
    return {
        _tag: "Form2051",
        capitalSocial,
        reserves,
...
// ============================================================================
export function computeForm2052(tb) {
    const exp = (prefix) => getDebitBalance(tb, prefix);
    ...
    const totalCharges = add(add(add(add(achatsMarchandises, variationStockMarchandises), add(achatsMatieresPremieres, variationStockMatieres)), add(add(autresAchatsChargesExternes, impotsTaxes), add(salaires, chargesSociales))), add(add(dotationsAmortissementsProvisions, autresCharges), add(add(chargesFinancieres, chargesExceptionnelles), impotBenefices)));
    return {
        _tag: "Form2052",
        achatsMarchandises,
...
// ============================================================================
export function computeForm2053(tb) {
    const rev = (prefix) => getCreditBalance(tb, prefix);
    ...
    const totalProduits = add(add(add(add(ventesMarchandises, productionVendueBiens), add(productionVendueServices, productionStockee)), add(productionImmobilisee, subventionsExploitation)), add(add(reprisesProvisions, autresProduits), add(produitsFinanciers, produitsExceptionnels)));
...
export function computeForm2058A(tb, adjustments) {
    // Résultat comptable from 2053
    const form2053 = computeForm2053(tb);
    const resultatComptable = form2053.resultat;
    const totalReintegrations = add(add(add(adjustments.remunerationExploitant, adjustments.chargesNonDeductibles), add(adjustments.amortissementsExcedentaires, adjustments.provisionsNonDeductibles)), adjustments.autresReintegrations);
    const totalDeductions = add(add(adjustments.produitsNonImposables, adjustments.deficitsAnterieurs), adjustments.autresDeductions);
 ...
 */
function toActifLineValue(line) {
    return {
        brut: line.brut,
 ...
 */
function toPassifLineValue(line) {
    return {
 ...
 */
export function form2050ToCerfa(form) {
    return {
 ...
 */
export function form2051ToCerfa(form) {
    return {
 ...
 */
export function form2052ToCerfa(form) {
    const mv = (m) => ({ montant: m });
    return {
 ...
 */
export function form2053ToCerfa(form) {
    const mv = (m) => ({ montant: m });
    return {
