...
export const PCG_ACCOUNTS = [
    ...
    {
        code: "781",
        name: "Reprises sur amortissements",
        class: 7,
        typeId: "REVENUE",
        isDebitNormal: false,
    },
...
export function getPcgAccountsForInsert() {
...
export function getAccountDefinition(code) {
    return PCG_ACCOUNTS.find((acc) => acc.code === code);
}
export function getRevenueAccountCodes() {
    return PCG_ACCOUNTS.filter((acc) => acc.class === 7).map((acc) => acc.code);
}
export function getExpenseAccountCodes() {
 ...
 */
export function getVatDeductibleAccount(expenseAccountCode) {
