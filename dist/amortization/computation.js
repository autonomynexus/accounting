...
function mul(a, rate) {
    return multiply(a, { amount: Math.round(rate * 10000), scale: 4 });
}
 ...
 */
export function computeProrata(dateDebut, dateFin, methode) {
    if (methode === "DEGRESSIF") {
        const months = (dateFin.getFullYear() - dateDebut.getFullYear()) * 12 +
...
// Linear Amortization
// ============================================================================
export function computeAmortissementLineaire(immo, exerciceDates) {
    ...
    for (const exercice of exerciceDates) {
        if (exercice.dateFin < immo.dateMiseEnService)
            continue;
        if (!greaterThan(vnc, immo.valeurResiduelle))
            break;
        const dateDebutAmort = exercice.dateDebut < immo.dateMiseEnService ? immo.dateMiseEnService : exercice.dateDebut;
...
// ============================================================================
export function computeAmortissementDegressif(immo, exerciceDates) {
    ...
    for (const exercice of exerciceDates) {
        ...
        if (!greaterThan(vnc, immo.valeurResiduelle) || anneesRestantes <= 0)
            break;
        const dateDebutAmort = exercice.dateDebut < immo.dateMiseEnService ? immo.dateMiseEnService : exercice.dateDebut;
...
// Asset Disposal
// ============================================================================
export function computeCession(immo, schedule, dateCession, prixCession) {
