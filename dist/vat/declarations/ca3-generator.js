...
import { VAT_COLLECTED_ACCOUNT, OTHER_TAXES_ACCOUNT, VAT_DEDUCTIBLE_ABS, VAT_DEDUCTIBLE_IMMOS, } from "../../chart-of-accounts.js";
...
import { calculateLigne29Total, } from "../annexe-a-models.js";
// ============================================================================
// Tagged Errors
// ============================================================================
export class Ca3GenerationError extends Schema.TaggedError()("Ca3GenerationError", { message: Schema.String }) {
}
...
// ============================================================================
export const Ca3GeneratorServiceLayer = Layer.effect(Ca3GeneratorService, Effect.gen(function* () {
    const journalData = yield* JournalDataPort;
    const getLines = (userId, period) => Effect.gen(function* () {
    ...
    const generate = (input) => Effect.gen(function* () {
        ...
        if (allLines.length === 0)
            return createEmptyCa3(previousCredit);
        const revenueByRate = {
            TVA20: zeroMonetary(),
            TVA10: zeroMonetary(),
            TVA55: zeroMonetary(),
        ...
        if (revenueByRate.other.amount !== 0 || vatCollectedByRate.other.amount !== 0) {
            return yield* new Ca3GenerationError({
                message: "Unsupported VAT rates detected. Check that all transactions use valid French VAT rates.",
            });
        }
        const ligne01 = Object.values(revenueByRate).reduce((sum, v) => add(sum, v), zeroMonetary());
        const ligne08 = { base: revenueByRate.TVA20, tva: vatCollectedByRate.TVA20 };
        const ligne09 = { base: revenueByRate.TVA55, tva: vatCollectedByRate.TVA55 };
        const ligne9B = { base: revenueByRate.TVA10, tva: vatCollectedByRate.TVA10 };
        const ligne10 = { base: revenueByRate.DOM, tva: vatCollectedByRate.DOM };
        const ligne11 = { base: revenueByRate.TVA21, tva: vatCollectedByRate.TVA21 };
        const ligne16 = [ligne08.tva, ligne09.tva, ligne9B.tva, ligne10.tva, ligne11.tva].reduce((sum, v) => add(sum, v), zeroMonetary());
    ...
    const generateTer = (input) => Effect.gen(function* () {
        ...
        const terSectors = sectors.map((s) => ({
            id: s.id,
            description: s.name,
        ...
        const vatDeductible = sectors.map((s) => {
            const acc = sectorAccumulators.get(s.id) ?? emptyAcc;
            return {
                sectorId: s.id,
    ...
    const generateAnnexeA = (input) => Effect.gen(function* () {
        const { userId, period } = input;
        const allLines = yield* getLines(userId, period);
        if (allLines.length === 0)
            return null;
        const taxLines = allLines.filter((line) => line.accountCode === OTHER_TAXES_ACCOUNT && line.taxeAssimileeType && line.creditAmount);
    ...
    const generateTic = (input) => Effect.gen(function* () {
        const { userId, period } = input;
        const allLines = yield* getLines(userId, period);
        if (allLines.length === 0)
            return null;
        const acciseLines = allLines.filter((line) => line.accountCode === OTHER_TAXES_ACCOUNT && line.acciseType && line.creditAmount);
        if (acciseLines.length === 0)
            return null;
        const acciseAccumulators = new Map();
        ...
        const mkSection = (type, acciseType, amount) => ({
            type,
            acciseType,
        ...
        const electricity = mkSection("ELECTRICITY", "TICFE", ticfeAmount);
        const gas = mkSection("GAS", "TICGN", ticgnAmount);
        const coal = mkSection("COAL", "TICC", ticcAmount);
        const other = mkSection("OTHER", "TICPE", ticpeAmount);
        const totalNetDue = ticfeAmount.amount + ticgnAmount.amount + ticcAmount.amount + ticpeAmount.amount;
        return {
            ...
            coal,
            other,
            netBalanceDue: totalNetDue >= 0 ? monetary({ amount: totalNetDue, currency: EUR }) : zeroMonetary(),
            netCreditRefund: totalNetDue < 0
...
// Helper Functions
// ============================================================================
function createEmptyTer(sectors) {
    const terSectors = sectors.map((s) => ({
        id: s.id,
        description: s.name,
        deductionPercentage: s.deductionPercentage,
...
function calculateTerTotals(vatBrute, vatDeductible, results) {
    let tvaBruteT = zeroMonetary(), tvaAReverser = zeroMonetary(), totalBrute = zeroMonetary();
    let immoTotal = zeroMonetary(), absTotal = zeroMonetary();
    let complementTvaDeductible = zeroMonetary(), totalTvaDeductible = zeroMonetary();
    let tvaNette = zeroMonetary(), creditTva = zeroMonetary();
...
function calculateGeneralDeductionPercentage(sectors, vatBrute) {
    ...
    for (const sector of sectors) {
        ...
        if (brute)
            weighted += sector.deductionPercentage * brute.tvaBrute.amount;
    }
    return weighted / totalVat;
}
function createEmptyCa3(previousCredit) {
...
//# sourceMappingURL=ca3-generator.js.map