...
export function getAutomaticVatRate(category) {
    if (category.pcg_account && VAT_EXEMPT_PCG_CODES.includes(category.pcg_account))
        return 0;
    if (VAT_EXEMPT_CATEGORY_NAMES.includes(category.name))
        return 0;
    return null;
}
...
export function getTaxRateOptions(t) {
    return [
