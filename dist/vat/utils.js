...
const DOM_TOM_VAT_CODES = new Set(["TVA85", "TVA21_DOM", "TVA175", "TVA105"]);
export function isValidVatCode(code) {
    return code !== null && code !== undefined && VALID_VAT_CODES.has(code);
}
export function isDomTomVatCode(code) {
...
const VAT_RATES_SCALE4 = {
    175: "TVA175",
    105: "TVA105",
    850: "TVA85",
};
function normalizeToScale3(rate) {
    const targetScale = 3;
    if (rate.scale === targetScale)
        return rate.amount;
    if (rate.scale < targetScale)
        return rate.amount * 10 ** (targetScale - rate.scale);
    return Math.round(rate.amount / 10 ** (rate.scale - targetScale));
}
function normalizeToScale4(rate) {
    const targetScale = 4;
    if (rate.scale === targetScale)
        return rate.amount;
    if (rate.scale < targetScale)
...
export function getVatInfo(taxRate, isRevenue) {
    const account = isRevenue ? VAT_COLLECTED_ACCOUNT : VAT_DEDUCTIBLE_ACCOUNT;
    if (!taxRate || taxRate.amount === 0)
        return { code: "TVA0", account };
...
export function hasVat(taxRate) {
    return taxRate !== null && taxRate !== undefined && taxRate.amount > 0;
}
export function getVatRatePercentage(code) {
    switch (code) {
        case "TVA20":
            return 20;
        case "TVA10":
