...
const A = "ASSET" as const; // Actif
const L = "LIABILITY" as const; // Passif (dette)
const E = "EQUITY" as const; // Capitaux propres
const R = "REVENUE" as const; // Produit
const X = "EXPENSE" as const; // Charge
...
export function getLeafAccounts(): readonly PcgAccountDefinition[] {
  const parentCodes = new Set(
    PCG_ACCOUNTS_FULL.map((a) => a.parentCode).filter((p): p is string => p !== null),
  );
