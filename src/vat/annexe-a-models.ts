 ...
 */
export const TAXE_ASSIMILEE_LABELS: Record<TaxeAssimileeType, string> = {
...
export type SimpleTaxLine = {
  ...
  readonly baseImposable: Monetary<number>; // Taxable base
  readonly taxeDue: Monetary<number>; // Calculated tax
};
...
export type TransportInfrastructureTax = SimpleTaxLine & {
  ...
  readonly details: {
    readonly tonnage: number; // Tonnage in tons
    readonly ratePerTon: Monetary<number>; // Rate per ton
  };
};
...
export type VideogramTax = SimpleTaxLine & {
  ...
  readonly details: {
    readonly advancesPaid: Monetary<number>; // Acomptes versés
    readonly unitsSold: number; // Number of units
...
export type ElectricityProductionMargin = SimpleTaxLine & {
  ...
  readonly details: {
    ...
    readonly marginRate: number; // Percentage
  };
};
...
export type ADeclaration = {
  ...
  readonly ligne29Total: Monetary<number>; // Sum of all taxes → CA3 ligne 29
};
...
export type ADeclarationInput = {
  ...
  readonly taxes: readonly {
    readonly taxType: TaxeAssimileeType;
    readonly enabled: boolean;
    readonly baseImposable: number; // cents
    readonly taxeDue: number; // cents
 ...
 */
export function toASnapshot(a: ADeclaration): ADeclarationSnapshot {
  const allTaxes: {
 ...
 */
export function fromASnapshot(snapshot: ADeclarationSnapshot): ADeclaration {
  const simpleTaxes: SimpleTaxLine[] = [];
  let transportInfrastructure: TransportInfrastructureTax | null = null;
  let videogram: VideogramTax | null = null;
  let electricityProduction: ElectricityProductionMargin | null = null;

  for (const tax of snapshot.taxes) {
    const baseTax = {
      taxType: tax.taxType as TaxeAssimileeType,
      enabled: tax.enabled,
 ...
 */
export function createEmptyADeclaration(): ADeclaration {
  return {
    taxes: [],
 ...
 */
export function calculateLigne29Total(a: ADeclaration): Monetary<number> {
  let total = 0;
